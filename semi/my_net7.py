...
parser.add_argument('--exp', type=str,
                    default='ACDC/7_vit', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    #model1 = create_model()
    #model2 = create_model()
    model = net_factory(net_type='ViT_Seg', in_chns=1, class_num=num_classes).cuda()
    model1 = net_factory(net_type='ViT_Seg', in_chns=1, class_num=num_classes).cuda()
    model2 = net_factory(net_type='ViT_Seg', in_chns=1, class_num=num_classes).cuda()
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            supervised_loss1 = (loss1_ce + loss1_dice) / 2
            ...
            supervised_loss2 = (loss2_ce + loss2_dice ) / 2
            ...
            model1_loss = (loss1 + supervised_loss1)/2 + consistency_weight * (pseudo_supervision1 + consistency_loss + unlabeled_rot_consistency_loss)
            model2_loss = (loss2 + supervised_loss2)/2 + consistency_weight * (pseudo_supervision2 + consistency_loss + unlabeled_rot_consistency_loss)
