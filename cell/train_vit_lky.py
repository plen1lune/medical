...
from vit import ViT, ViT_v2, MHA
...
net = ViT_v2(feature_size=16, patch_size=4, num_classes=3, dim=256, depth=10, heads=4,
          mlp_dim=512,pool='cls',dim_head=64)  # 一开始不能设置全连接层的输出种类为自己想要的，必须先将模型参数载入，再修改全连接层
resnet = resnet18(include_top=False)
res_prepath = "/mnt/nas/llc/acc80/path3/b/resNet18_new1_color_crop_acc0.929.pth" #"/mnt/nas/llc/acc80/path3/b/resNet18_new_color_acc0.885.pth"
...
for epoch in range(100):
    ...
    with torch.no_grad():
        ...
        if val_accurate >= best_acc and val_accurate > 0.8:
            if epoch > 15:
                learningrate = learningrate * decay
            json_context = {}
            ...
            json_path = os.path.join("./result/json_moredata", "ViT_adam_resnet_pretrainedv3_bs16_depth10_clstoken_%.3f.json" % val_accurate)
            best_acc = val_accurate
            save_path = './result/path_moredata/ViT_adam_resnet_pretrainedv3_bs16_depth10_clstoken_acc%.3f.pth' % val_accurate
            model_path = './result/model_moredata/ViT_adam_resnet_pretrainedv3_bs16_depth10_clstoken_acc%.3f.pth' % val_accurate
