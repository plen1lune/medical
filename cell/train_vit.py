...
net = ViT(image_size=512, patch_size=64, num_classes=3, dim=256, depth=6, heads=16,
          mlp_dim=512)  # 一开始不能设置全连接层的输出种类为自己想要的，必须先将模型参数载入，再修改全连接层
...
for epoch in range(100):
    ...
    with torch.no_grad():
        ...
        if val_accurate > best_acc and val_accurate > 0.85:
            ...
            json_path = os.path.join("./cl3/b", "ViT_2m0.5_%.3f.json" % val_accurate)
            best_acc = val_accurate
            save_path = './path3/b/ViT_2m0.5_acc%.3f.pth' % val_accurate
            model_path = './model3/b/ViT_2m0.5_acc%.3f.pth' % val_accurate
