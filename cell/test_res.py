...
image_path = data_root + "/ybr_testb/"  # data set path
...
# 官方提供载入预训练模型的方法
model_weight_path = "/home/xjl/llc/ybr/path/resnet34/resnet34_new12_acc0.873.pth"  # 1权重路径:预训练权重 512 结果：887
...
with torch.no_grad():
    ...
    if val_accurate > best_acc and val_accurate > 0.85:
        ...
        json_path = os.path.join("./result/json", "resnet34_trainb12_testub12_acc%.3f.json" % val_accurate)
        best_acc = val_accurate
    ...
    print("\n2D_A: %.3f " % (acc_val_m[3] / num_val_m[3]))
    print("\n2D_BX: %.3f " % (acc_val_m[4] / num_val_m[4]))
    print("\n2D_P: %.3f " % (acc_val_m[5] / num_val_m[5]))
    # print("\n2D_RVI: %.3f " % (acc_val_m[7] / num_val_m[7]))
    print("\n2D_SAA: %.3f " % (acc_val_m[6] / num_val_m[6]))
    print("\n2D_SAB: %.3f " % (acc_val_m[7] / num_val_m[7]))
    print("\n2D_SAM: %.3f " % (acc_val_m[8] / num_val_m[8]))
    print("\n2D_SAoA: %.3f " % (acc_val_m[9] / num_val_m[9]))
    # print("\n2D_SSF: %.3f " % (acc_val_m[12] / num_val_m[12]))
    print("\n3D: %.3f " % (acc_val_m[10] / num_val_m[10]))
    print("\nCDFI: %.3f " % (acc_val_m[11] / num_val_m[11]))
