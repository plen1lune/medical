...
image_path = data_root + "/image3_b_add3/"  # data set path
...
# 官方提供载入预训练模型的方法
model_weight_path = "/mnt/nas/llc/acc80/path3/b/resNet18_new1_color_crop_acc0.929.pth"  #"/mnt/nas/llc/acc80/path3/b/resNet18_new_color_acc0.885.pth" # 权重路径
resnet.load_state_dict(torch.load(model_weight_path), strict=False)  # 载入模型权重, strict=False
vit_path = "/mnt/nas/llc/acc80/result/path_moredata_ontest/ViT_adam_resnet_pretrainedv3_bs16_depth10_clstoken_acc0.960.pth"#"/mnt/nas/llc/acc80/result/path/VIT_resnet_pretrained_bs16_head1_depth10_clstoken_acc0.937.pth"
...
def visual_att(mapp_list, filename):
    ...
    for mapp in mapp_list:
        ...
        cc = out.shape[1]
        depths.append(out)
    f, axes = plt.subplots(figsize = (35, 75), nrows=len(mapp_list))
    #f, axes = plt.subplots(figsize = (35, 7), nrows=len(mapp_list))
    for i,res in enumerate(depths):
        #print (res.shape)
        res = res[:,1:]
        fig = sns.heatmap(res,ax = axes[i],cmap='Blues') #, cmap='RdBu_r',square=True,center=0
        #fig = sns.heatmap(res,ax = axes,cmap='Blues')  # when head =1
...
def visual_att_patch(mapp_list, sample_fname, dir_name):
    ...
    for k, mapp in enumerate(mapp_list):
        ...
        for h in range(mapp.shape[0]):
            ...
            filename = dir_name +'/'+ sample_fname.split('/')[-2]+'-'+sample_fname.split('/')[-1].split('.')[0]+'/layer{}_head{}'.format(k,h)+'.png'
...
def visual_att_patch_ave(mapp_list, sample_fname, dir_name):
    ...
    plt.plot()
    plt.imshow(pic_resize_griddata,interpolation='nearest', cmap='Blues') #, origin='upper'
    filename = dir_name +'/'+ sample_fname.split('/')[-2]+'-'+sample_fname.split('/')[-1].split('.')[0]+'/ave_att'+'.png'
    dirr = os.path.dirname(filename)
    if not os.path.exists(dirr):
        os.makedirs(dirr)
    plt.savefig(filename)
...
with torch.no_grad():
    ...
    pic_list = ["/mnt/nas/llc/acc80/image3_b_add3/test/light/4-13_058_l.jpeg","/mnt/nas/llc/acc80/image3_b_add3/test/light/4-17_001_d.jpeg",
                "/mnt/nas/llc/acc80/image3_b_add3/test/medium/4-29_003.jpeg","/mnt/nas/llc/acc80/image3_b_add3/test/medium/7_33_040.jpeg",
                "/mnt/nas/llc/acc80/image3_b_add3/test/medium/7_33_055.jpeg","/mnt/nas/llc/acc80/image3_b_add3/test/medium/9_68_051.jpeg",
                "/mnt/nas/llc/acc80/image3_b_add3/test/serious/6_71_055.jpeg"]

    """如果测试集的准确度比历史最佳还高，那么更新json文件，保存权重"""
    if val_accurate > best_acc:
        json_context = {}
        #json_path = os.path.join("/mnt/nas/llc/acc80/result/test_moredata", "ViT_adam_resnet_pretrainedv3_bs16_depth10_clstoken_0.960_%.3f.json" % val_accurate)
        save_dir = "/mnt/nas/llc/acc80/result/visual/ViT_adam_resnet_pretrainedv3_bs16_depth10_clstoken_acc0.960.pth"
        ...
        for step2, val_data in enumerate(validate_loader, start=0):
            ...
            for i in range(0, len(val_images.to(device))):
                ...
                val_sample_fname, _ = validate_loader.dataset.samples[step2 * batch_size + i]
                if val_sample_fname in pic_list: 
                    print(val_sample_fname)  # 打印文件名
                    
                    save_name = os.path.join(save_dir,val_sample_fname.split('/')[-2]+'-'+val_sample_fname.split('/')[-1].split('.')[0]+'.png')
                    visual_att(attn_list, save_name)
                    visual_att_patch(attn_list,val_sample_fname,save_dir)
                    visual_att_patch_ave(attn_list,val_sample_fname,save_dir)
