...
"""res_prepath = "/mnt/nas/llc/acc80/path3/b/resNet18_1_acc0.949.pth"
#checkpoint = torch.load(res_prepath)
...
for epoch in range(100):
    ...
    for step, (images, labels) in enumerate(train_loader, start=0):
        ...
        o = resnet(images.to(device))  # eval model only have last output layer
        outputs = net(o)
    ...
    with torch.no_grad():
        for step2, val_data in enumerate(validate_loader, start=0):
            ...
            feas1 = resnet(val_images.to(device))  # eval model only have last output layer
            outputs1 = net(feas1)
            # loss = loss_function(outputs, test_labels)
        ...
        if val_accurate > best_acc and val_accurate > 0.85:
            ...
            json_path = os.path.join("./result/json2", "fromscratch_resViT_bs1_depth10_clstoken_%.3f.json" % val_accurate)
            best_acc = val_accurate
            save_path1 = './result/path2/fromscratch_resnet_bs16_depth10_clstoken_acc%.3f.pth' % val_accurate
            model_path1 = './result/model2/fromscratch_resnet_bs16_depth10_clstoken_acc%.3f.pth' % val_accurate
            save_path2 = './result/path2/fromscratch_ViT_bs16_depth10_clstoken_acc%.3f.pth' % val_accurate
            model_path2 = './result/model2/fromscratch_ViT_bs16_depth10_clstoken_acc%.3f.pth' % val_accurate
