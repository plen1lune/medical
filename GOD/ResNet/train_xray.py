...
image_path = data_root + "/chest_classify/"  # data set path
...
# 官方提供载入预训练模型的方法
model_weight_path = "./weights/resnet18.pth"  # 1权重路径:预训练权重 512 结果：887
...
for epoch in range(100):
    ...
    for step, (images, labels) in enumerate(train_loader, start=0):
        ...
        num0 = metrix[0][0] + metrix[0][1]
        num1 = metrix[1][0] + metrix[1][1]
    ...
    with torch.no_grad():
        ...
        if val_accurate > best_acc and val_accurate > 0.85:
            ...
            json_path = os.path.join("./cl", "chest_%.3f.json" % val_accurate)
            best_acc = val_accurate
            save_path = './path/chest_acc%.3f.pth' % val_accurate
            # model_path = './model/b/resNet18_new1_color_crop_acc%.3f.pth' % val_accurate
