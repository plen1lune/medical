...
def save_individual_images(images, title="", save_path="", i=0):
    ...
    for idx, img in enumerate(images):
        ...
        plt.savefig(os.path.join(save_path, f"{title}_{idx}_{i}.png"))
...
def main():
    ...
    parser.add_argument("--class", type=int, default=0, help="the class of data we generate")
    parser.add_argument("--cuda", type=int, default=0, help="the class of data we generate")
    args = vars(parser.parse_args())
    print(args)

    # Model store path
    store_path = f'./weights/chest_train_120_class_{args["class"]}.pt'
    ...
    image_path = data_root + "/chest_xray/train/"  # data set path

    if args["class"] == 0:
        train_dataset = datasets.ImageFolder(root=image_path + "n",
                                             transform=transform)
    elif args["class"] == 1:
        train_dataset = datasets.ImageFolder(root=image_path + "p",
                                             transform=transform)
    else:
        train_dataset = datasets.ImageFolder(root=image_path + "serious",
                                             transform=transform)
    ...
    for i in range(10):
        ...
        # show_images(generated, "Final result", f"./med_new_class/")
        save_individual_images(generated, "Final result", f'./chest_resulttest/{args["class"]}', i)
